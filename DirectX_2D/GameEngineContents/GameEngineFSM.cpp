...
void GameEngineFSM::CreateState(const StateParameter& _StateFunction)
{
	...
	{
		MsgBoxAssert("이미 존재하는 이름의 스테이트를 또 만들려고 했습니다." + std::string(_StateFunction.Name));
	}

	std::string UpperName = GameEngineString::ToUpperReturn(_StateFunction.Name);
...
)
{
	...
	{
		MsgBoxAssert("이미 존재하는 이름의 스테이트를 또 만들려고 했습니다." + std::string(_Name));
	}
...
void GameEngineFSM::Update(float _DeltaTime)
{
	if (nullptr == CurState)
	{
		MsgBoxAssert("현재 스테이트가 존재하지 않습니다");
	}
...
void GameEngineFSM::ChangeState(const std::string_view& _State) 
{
	...
	if (nullptr == CurState)
	{
		MsgBoxAssert("존재하지 않는 스테이트로 이동하려고 했습니다" + std::string(_State));
	}
